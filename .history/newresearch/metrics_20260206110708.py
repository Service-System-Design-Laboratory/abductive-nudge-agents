...
def extract_metrics(run_dir: Path) -> dict[str, Any] | None:
    ...
    if judge_path.exists() and decision_path.exists():
        ...
        if judgements:
            ...
            if contrasting_id:
                sel_h = next((h for h in hypotheses if h.get("hypothesis_id") == selected_id), None) if hypotheses_data else None
                con_h = next((h for h in hypotheses if h.get("hypothesis_id") == contrasting_id), None) if hypotheses_data else None
