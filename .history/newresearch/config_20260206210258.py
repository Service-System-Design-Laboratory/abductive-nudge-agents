...
class RunConfig:
    ...
    def __post_init__(self):
        if not self.run_id:
            ...
            if self.is_single:
                self.run_id = f"{ts}__{self.persona_id}__{self.scenario_id}__SINGLE"
            else:
                pkg = "1" if self.use_pkg else "0"
                rag = "1" if self.use_rag else "0"
                judge = "1" if self.use_judge else "0"
                self.run_id = f"{ts}__{self.persona_id}__{self.scenario_id}__PKG{pkg}_RAG{rag}_JUDGE{judge}"
...
def make_config(scenario_id: str, condition: str, persona_id: str = "P01") -> RunConfig:
    """Create a RunConfig from scenario ID, condition letter, and persona ID."""
