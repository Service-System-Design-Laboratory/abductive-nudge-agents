...
def fig1_reframing_rate_by_condition(records):
    ...
    ax.set_ylim(0, 115)
    ax.set_title('Fig 1. Selected-Hypothesis Reframing Rate by Ablation Condition')
...
def fig2_reframing_by_persona(records):
    ...
    for i, c in enumerate(conds):
        ...
        bars = ax.bar(x + i * width, vals, width, label=COND_LABELS[c].replace('\n', ' '),
                      color=COND_COLORS[c], edgecolor='white')
    ...
    ax.set_ylabel('Reframing Rate (%)')
    ax.set_ylim(0, 115)
    ax.set_title('Fig 2. Reframing Rate by Persona × Condition')
...
def fig3_scenario_heatmap(records):
    ...
    ax.set_title('Fig 3. Reframing Rate Heatmap: Scenario × Condition')
...
def fig4_link_rates(records):
    ...
    ax1.set_ylabel('Rate (%)')
    ax1.set_ylim(0, 115)
    ax1.set_title('persona_link Present')
    ...
    ax2.set_ylabel('Rate (%)')
    ax2.set_ylim(0, 115)
    ax2.set_title('evidence_link Present')
    ax2.spines['top'].set_visible(False)
    ax2.spines['right'].set_visible(False)

    fig.suptitle('Fig 4. Grounding Link Rates by Condition', fontsize=13, y=1.02)
    fig.tight_layout()
    return fig
...
def fig5_hypothesis_composition(records):
    ...
    bars1 = ax.bar(range(4), nonrefr_avgs, color='#90CAF9', edgecolor='white',
                   label='Non-reframing', width=0.6)
    bars2 = ax.bar(range(4), refr_avgs, bottom=nonrefr_avgs, color='#1565C0',
                   edgecolor='white', label='Reframing', width=0.6)

    for i in range(4):
        total = refr_avgs[i] + nonrefr_avgs[i]
        ax.text(i, total + 0.1, f'{total:.1f}', ha='center', fontsize=10)
    ...
    ax.set_ylim(0, 7)
    ax.set_title('Fig 5. Hypothesis Composition by Condition')
    ax.legend()
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    fig.tight_layout()
...
def fig6_persona_scenario_detail(records):
    ...
    # Legend
    green_patch = mpatches.Patch(color='#2ca02c', label='Reframing (✓)')
    red_patch   = mpatches.Patch(color='#d62728', label='Non-reframing (✗)')
    fig.legend(handles=[green_patch, red_patch], loc='lower center',
               ncol=2, fontsize=10, bbox_to_anchor=(0.5, -0.02))
    fig.suptitle('Fig 6. Per-Persona Reframing Detail: Scenario × Condition', fontsize=13)
...
def fig7_summary_table(records):
    ...
    ax.set_title('Fig 7. Summary Table: All Metrics by Condition', fontsize=13, pad=20)
    fig.tight_layout()
    return fig
...
def fig8_condition_comparison_radar(records):
    ...
    ax.set_title('Fig 8. Multi-Dimensional Condition Comparison', fontsize=13, pad=20)
