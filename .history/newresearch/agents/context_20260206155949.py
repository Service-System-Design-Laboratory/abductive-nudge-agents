...
class ContextAgent(BaseAgent):
    ...
    def process(self, state: dict) -> dict:
        ...
        # Write observations & assumptions to PKG (dynamic graph update)
        if use_pkg and pkg_store:
            run_id = state.get("config", {}).run_id if hasattr(state.get("config"), "run_id") else ""
            pkg_store.write_observations(
