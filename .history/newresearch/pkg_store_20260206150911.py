...
class PKGStore:
    ...
    def _next_node_id(self, session) -> str:
        """Generate next sequential node_id (e.g. n11, n12, ...)."""
        result = session.run(
            """
            MATCH (u:ExpUser {persona_id: $pid})-[:HAS_PKG_NODE]->(n:PKGNode)
            RETURN n.node_id AS nid ORDER BY n.node_id DESC LIMIT 1
            """,
            pid=self.persona_id,
        ).single()
        if result:
            num = int(result["nid"].lstrip("n")) + 1
        else:
            num = 1
        return f"n{num}"
