...
def fig_a_judge_scores(metrics: list[dict]):
    """Fig. A: Judge diagnostic quality — explains_confirmed, fatal_flaw_avoided, discriminating_q coverage."""
    _setup_japanese_font()

    # Only conditions with judge (A, B, C)
    rows = [m for m in metrics if m.get("fatal_flaw_avoided") is not None]
    if not rows:
        logger.warning("No judge data for Fig A")
        return

    judge_metrics = ["fatal_flaw_avoided", "selected_is_max_explains", "discriminating_q_coverage", "has_contrasting", "contrast_type_diff"]
    metric_labels = ["Fatal Flaw\nAvoided", "Max Explains\nSelected", "Discrim. Q\nCoverage", "Has\nContrasting", "Contrast\nType Diff"]
    ...
    for i, cond in enumerate(conditions):
        ...
        for jm in judge_metrics:
            vals = [float(r.get(jm, 0) or 0) for r in cond_rows]
            means.append(np.mean(vals) if vals else 0)
        ax.bar(x + offsets[i], means, width * 0.9,
               label=CONDITION_LABELS.get(cond, cond),
               color=CONDITION_COLORS.get(cond, "#999"))

    ax.set_xlabel("Judge Diagnostic Metric")
    ax.set_ylabel("Rate (0-1)")
    ax.set_title("Fig. A: Judge Diagnostic Quality by Condition")
    ax.set_xticks(x)
