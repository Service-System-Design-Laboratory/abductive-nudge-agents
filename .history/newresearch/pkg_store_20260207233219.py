...
class PKGStore:
    ...
    def __init__(self, persona_id: str = "P1", enabled: bool = True, write_enabled: bool = False):
    ...
    def verify_persona_links(self, node_ids: list[str]) -> str:
        ...
        Strips 'pkg:' prefix if present (LLM may emit 'pkg:p01_n3')."""
        if not self.enabled or not self._driver:
            return "(実験条件によりPKG無効)"

        # Normalize: strip 'pkg:' prefix that LLM may hallucinate
        node_ids = [nid.replace("pkg:", "") for nid in node_ids]
    ...
    def _init_counter(self):
        ...
        Handles both 'n5' (runtime) and 'p01_n1' (seed) ID formats.
        """
        import re
        with self._driver.session() as s:
            records = s.run(
        ...
        for r in records:
            nid = r["nid"]
            # Extract trailing number from both 'n5' and 'p01_n5' formats
            m = re.search(r"(\d+)$", nid)
            if m:
                num = int(m.group(1))
                if num > max_num:
        ...
        logger.info(f"[PKGStore] initialized counter at n{max_num} (persona={self.persona_id})")
    ...
    def write_observations(self, observations: list[dict], assumptions: list[dict],
        ...
        if not self.enabled or not self._driver or not self.write_enabled:
            return []
    ...
    def write_hypotheses(self, hypotheses: list[dict], run_id: str) -> list[str]:
        ...
        with self._driver.session() as s:
            for hyp in hypotheses:
                nid = self._next_node_id()
                # Use 'statement' field (actual schema), fallback to 'text'
                label = (hyp.get("statement") or hyp.get("text") or "")[:150]
                s.run(
                    """
                    MATCH (u:ExpUser {persona_id: $pid})
                    CREATE (n:PKGNode {
                ...
                # Link to observation nodes this hypothesis explains
                for obs_id in hyp.get("explains", []):
                    s.run(
                        """
                        MATCH (u:ExpUser {persona_id: $pid})-[:HAS_PKG_NODE]->(hyp:PKGNode {node_id: $nid})
                        MATCH (u)-[:HAS_PKG_NODE]->(obs:PKGNode)
                        WHERE obs.type = 'observation' AND obs.run_id = $run_id
                        WITH hyp, obs ORDER BY obs.node_id LIMIT 1
    ...
    def write_judgements(self, judgements: list[dict], run_id: str) -> list[str]:
        ...
        with self._driver.session() as s:
            for j in judgements:
                nid = self._next_node_id()
                # 'diagnosis' is a dict — extract key info for label
                diag = j.get("diagnosis", {})
                if isinstance(diag, dict):
                    flaws = diag.get("fatal_flaws", [])
                    label = f"致命的欠陥: {'; '.join(flaws)}" if flaws else diag.get("testability_notes", "")[:150]
                else:
                    label = str(diag)[:150]
                s.run(
                    """
    ...
    def reset_to_seed(self):
        """Delete ALL runtime-generated nodes (those with a 'source' property),
        keeping only the original seed nodes. Re-initializes the node counter.
        Must be called before each experiment run to prevent cross-run contamination."""
        if not self.enabled or not self._driver:
            return
        with self._driver.session() as s:
            result = s.run(
            ...
            count = result["deleted"] if result else 0
        # Re-initialize counter from remaining (seed) nodes
        self._init_counter()
        logger.info(f"[PKGStore] reset_to_seed({self.persona_id}): deleted {count} runtime nodes, counter reset to n{self._node_counter}")
